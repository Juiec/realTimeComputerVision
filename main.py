...
import addons  # Import shared code and models from addons.py
    ...
    if mode in (MODE_POSE, MODE_BOTH, MODE_ALL) and pose_model is not None:
        ...
        if pose_results.keypoints is not None and pose_results.keypoints.xy is not None:
            ...
            for xy in xy_all:  # loop over each person
                right_angle = addons.get_forearm_angle(xy, side="right")
                left_angle = addons.get_forearm_angle(xy, side="left")

                if not hasattr(detect_objects, "prev_right_angle"):
                    detect_objects.prev_right_angle = None
                if not hasattr(detect_objects, "prev_left_angle"):
                    detect_objects.prev_left_angle = None

                '''
                # Hand raised
                if addons.is_hand_raised(xy):
                    cv2.putText(frame, "HAND RAISED", (50, 120),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 3)

                # Pointing
                if addons.is_pointing(xy):
                    cv2.putText(frame, "POINTING", (50, 170),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 3)
                '''
                right_feedback = addons.get_hand_turn_feedback(
                    right_angle,
                    detect_objects.prev_right_angle,
                    )

                if right_feedback is not None:
                    turn_feedback.append(right_feedback)
                    cv2.putText(frame, right_feedback, (50, 220),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 100, 0), 3)

                left_feedback = addons.get_hand_turn_feedback(
                    left_angle,
                    detect_objects.prev_left_angle,
                )
                if left_feedback is not None:
                    turn_feedback.append(left_feedback + " (L)")
                    cv2.putText(frame, left_feedback + " (L)", (50, 270),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 100, 0), 3)

                detect_objects.prev_right_angle = right_angle
                detect_objects.prev_left_angle = left_angle
